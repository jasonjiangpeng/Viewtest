...
public class SimpleViewer2 extends Thread {
    ...
    public SimpleViewer2(ImiDevice device, ImiFrameType frameType) {
        mDevice = device;
        mFrameType = frameType;
    }
    public SimpleViewer2(ImiDevice device, ImiFrameType frameType, Handler handler) {
        mDevice = device;
        mFrameType = frameType;
        mhandler = handler;
    }
    ...
    public void run() {
        ...
        while (mShouldRun) {
            ...
            if (original==null){
                ...
                short[][]  shd=trans2short(comdata);

                ToolsList toolsd = compareShort(original, shd);
                int  c=toolsd.getValue();
                int  d=toolsd.getTpye();
